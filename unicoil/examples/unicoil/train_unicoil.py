...
def main():
    ...
    if data_args.training_method not in ['cross_entropy', 'kl_div', 'margin_mse']:
        assert False

    if data_args.training_method != "cross_entropy" and "distill" not in data_args.train_dir:
        assert False, "selected training file does not contain teacher scores!"
    ...
    # disabled_token_ids = tokenizer.convert_tokens_to_ids(["[SEP]", "[CLS]", "[MASK]", "[PAD]", "[UNK]"])
    ...
    model = UniCoilModel.build(
        ...
        config=config,
        cache_dir=model_args.cache_dir,
        training_method=data_args.training_method,
